...
class SchemaAnalyzer:
    ...
    def get_sample_files(self, bucket_name: str) -> List[str]:
        ...
        try:
            ...
            print(f"Selected {len(selected_files)} files for analysis: {selected_files}")
    ...
    def analyze_json_structure(self, data: Dict[str, Any], prefix: str = "") -> Dict[str, Set[str]]:
        """
        ...
        for key, value in data.items():
            ...
            if ((prefix == "gameData.players" or 
                 prefix.endswith(".boxscore.teams.away.players") or 
                 prefix.endswith(".boxscore.teams.home.players")) and 
                key.startswith("ID")):
            ...
            if isinstance(value, dict):
                ...
                if value:  # Analyze first item if list is not empty
                    if isinstance(value[0], dict):
                        nested_schema = self.analyze_json_structure(value[0], f"{full_path}[]")
                        schema.update(nested_schema)
                    else:
    ...
    def generate_schema_documentation(self, schema: Dict[str, Set[str]], bucket_name: str) -> None:
        """
    ...
    def save_sample_json(self, data: Dict[str, Any], bucket_name: str, file_name: str) -> None:
        """
