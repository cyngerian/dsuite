...
class MigrationGenerator:
    ...
    def _find_modified_tables(self) -> Dict[str, TableDiff]:
        ...
        modified = {}
        common_tables = set(self._get_all_tables(self.old_schema)) & set(
            self._get_all_tables(self.new_schema)
        )
    ...
    def _compare_tables(self, old_table: Dict, new_table: Dict) -> TableDiff:
        ...
        removed_cols = {k for k in old_cols if k not in new_cols}
        modified_cols = {
            k: v for k, v in new_cols.items() if k in old_cols and v != old_cols[k]
        }
    ...
    def _generate_drop_statements(self, tables: Dict) -> List[str]:
        ...
        return [f"DROP TABLE IF EXISTS {table} CASCADE;" for table in tables]
    ...
    def _generate_create_statements(self, tables: Dict) -> List[str]:
        ...
        return [
            sql_gen.generate_create_table(name, table) for name, table in tables.items()
        ]
